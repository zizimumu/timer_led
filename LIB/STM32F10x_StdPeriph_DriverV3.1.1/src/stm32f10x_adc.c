...
void ADC_UserInit(void )
{
	...
	/* Configure ADCCLK such as ADCCLK = PCLK2/6 ,max clock is 14Mhz */ 
	...
	ADC_RegularChannelConfig(ADC1, ADC_Channel_0, 1, ADC_SampleTime_71Cycles5 );	//ADC1,ADC通道0,规则采样顺序值为4,采样时间为239.5周期	
	
	/* Enable ADC1 */
	ADC_Cmd(ADC1, ENABLE);
	

	ADC_ResetCalibration(ADC1);	/* reset calibaration register */   
...
uint16_t ADC_StartOneConv(void )
{
