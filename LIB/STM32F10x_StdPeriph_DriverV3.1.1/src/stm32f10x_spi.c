...
/*
note: a complete process : write and wait it have done
*/
u8 SPI_UserReadWriteByte(u8 TxData)
{		
	uint32_t retry=0;	
	u8 tmp;
	...
	while( (SPI1->SR & SPI_I2S_FLAG_RXNE) == RESET ){
		...
		retry++;
		if(retry == 20000){
			DebugPrint("SPI read byte failed\r\n");
			return 0;
		}
	}
	tmp = (u8)SPI1->DR;
...
void SPI_UserWriteByte(u8 TxData)
{					 
	while( (SPI1->SR & SPI_I2S_FLAG_TXE) == RESET);
	SPI1->DR = TxData; 	    
}

u8 SPI2_UserWriteByte(u8 TxData)
{					 
	
	 
	while( (SPI2->SR & SPI_I2S_FLAG_TXE) == RESET);
	SPI2->DR = TxData;  
	while( (SPI2->SR & SPI_I2S_FLAG_TXE) ==SET);
