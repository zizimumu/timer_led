...
void I2c_UserInit(void)
{
	...
	/* GPIO_Pin_10 -> SCL   GPIO_Pin_11-> SDA*********************/
...
void IIC_Stop_IO(void)
{
	...
	Clr_IIC_SDA();//STOP:when CLK is high DATA change form low to high
 	delay_us(4);
	Set_IIC_SCL(); 
	Set_IIC_SDA();//·¢ËÍI2C×ÜÏß½áÊøÐÅºÅ
	delay_us(4);							   	
}
//µÈ´ýÓ¦´ðÐÅºÅµ½À´
//·µ»ØÖµ£º1£¬½ÓÊÕÓ¦´ðÊ§°Ü
//        0£¬½ÓÊÕÓ¦´ð³É¹¦
u8 IIC_Wait_Ack_IO(void)
{
	u8 ucErrTime=0;
...
u8 IIC_Read_Byte_IO(unsigned char ack)
{
